...
export async function createExpressApp() {
  ...
  // ── Request Changes (customer-initiated from /proposal/:id) ────────────────
  // Customer describes what they want adjusted. We log the request as an
  // activity on the submission and email the admin with full context.
  app.post("/api/request-changes", async (req: Request, res: Response) => {
    try {
      const { submissionId, message } = req.body;
      if (!submissionId || !message || typeof message !== "string") {
        return res.status(400).json({ success: false, error: "Missing submissionId or message" });
      }
      ...
      const sub = snap.data() || {};
      const customerName  = sub.name  || "Customer";
      const customerEmail = sub.email || "";

      // Log activity on the submission's timeline so the admin sees it
      // alongside email events, views, etc.
      await adminDb
        .collection("submissions")
        .doc(submissionId)
        .collection("activities")
        .add({
          ...
          actor: "customer",
          meta: { fullMessage: trimmed },
          createdAt: FieldValue.serverTimestamp(),
        });

      // Also bump a top-level field for quick admin-side awareness.
      await adminDb
        .collection("submissions")
        .doc(submissionId)
        .update({
          lastChangeRequestAt: FieldValue.serverTimestamp(),
        });

      // Email the admin with the change request.
      if (resend) {
        const origin = req.headers.origin || req.headers.referer || "https://eclipsepergola.netlify.app";
        const proposalLink = `${origin}/proposal/${submissionId}`;
        const escaped = trimmed.replace(/</g, "&lt;").replace(/>/g, "&gt;");
        try {
          await resend.emails.send({
            from: FROM_EMAIL,
            to: ADMIN_EMAIL,
            subject: `✏️ Change request from ${customerName}`,
            tags: [{ name: "submissionId", value: String(submissionId) }],
            html: `
              <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:640px;margin:0 auto;padding:24px;color:#1A1A1A;line-height:1.55;">
                <h1 style="color:#1A1A1A;border-bottom:3px solid #C5A059;padding-bottom:12px;font-size:22px;">Change Request</h1>
                <p><strong>From:</strong> ${customerName}${customerEmail ? ` &lt;${customerEmail}&gt;` : ""}</p>
                <p><strong>Submission:</strong> <code>${submissionId}</code></p>
                <h2 style="color:#C5A059;font-size:15px;margin-top:24px;">What they'd like changed</h2>
                <blockquote style="border-left:3px solid #C5A059;padding:4px 16px;margin:8px 0;color:#333;white-space:pre-wrap;background:#fafafa;">${escaped}</blockquote>
                <div style="margin:24px 0;">
                  <a href="${proposalLink}" style="display:inline-block;background:#1A1A1A;color:#fff;font-weight:700;padding:12px 28px;border-radius:8px;text-decoration:none;font-size:14px;">Open proposal</a>
                </div>
                <p style="color:#999;font-size:11px;margin-top:32px;border-top:1px solid #eee;padding-top:12px;">
                  Reply directly to revise pricing, or open the admin CRM to tweak the configuration.
                </p>
              </div>
            `,
          });
        } catch (e) {
          console.error("request-changes: admin email failed", e);
          // Activity is already logged; don't fail the whole request because
          // the email send hiccuped.
        }
      }
